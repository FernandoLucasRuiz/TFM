...
subtitle: "EXTREME SAMPLE DETECTION MECHANISM"
...
# Load data from the filtering mechanism
...
# EXTREME SAMPLE DETECTION MECHANISM
...
### Select extreme samples for the first two pcs.

I select the outliers of the first two PCs with double SD.
...
```{r}
# Make sure that all vectors are the same length for the final dataframe.
max_length <- max(length(mPC1.pos),
                  length(mPC1.neg),
                  length(mPC2.pos), 
                  length(mPC2.neg))

# Normalize the length of the vectors (in case one of them is shorter)
...
```{r}
save.image("EXTREME SAMPLE DETECTION MECHANISM.RData")
```
