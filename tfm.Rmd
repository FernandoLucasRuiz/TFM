...
plot2 <- ggplot(umap.data, aes(x = V2)) +
  geom_density(fill = "#00CED1", alpha = 0.5) +
  geom_vline(xintercept = mean(umap.data$V2) + 2*sd.umap2, linetype = "dashed", color = "blue") +
  geom_vline(xintercept = mean(umap.data$V2) - 2*sd.umap2, linetype = "dashed", color = "blue") +
...
for (i in rownames(covs)){
  if (i %in% rownames(mUMAP1) & !(i %in% rownames(mUMAP12))){
    covs[i, "sampleset_UMAP"] <- "mUMAP1"
  }
  if (i %in% rownames(mUMAP2) & !(i %in% rownames(mUMAP12))){
    covs[i, "sampleset_UMAP"] <- "mUMAP2"
  }
  if (i %in% rownames(mUMAP12)){
    covs[i, "sampleset_UMAP"] <- "mUMAP1 and mUMAP2"
  }
  if (!(i %in% rownames(mUMAP1)) & !(i %in% rownames(mUMAP2))){
    covs[i, "sampleset_UMAP"] <- "Not in both"
  }
}

covs$sampleset_UMAP <- as.factor(covs$sampleset_UMAP)
table(covs$sampleset_UMAP)
```
...
for (i in 1:ncol(covs)) {
  ...
  if (class(covs[[i]]) == "factor"){
    p <- covs %>%
      ...
      summarise(count = n(), .groups = 'drop') %>%
      mutate(total = sum(count), percentage = (count / total) * 100) %>%
      ggplot(aes_string(x = names(covs)[i], y = "percentage", fill = "sampleset_UMAP")) +
      geom_bar(stat = "identity", position = "fill") +
      labs(x = NULL, y = "%", fill = col_name) +
      theme_minimal()
  ...
  if (class(covs[[i]]) == "numeric"){
    p <- ggplot(covs, aes_string(y=names(covs)[i], x = "sampleset_UMAP", fill = "sampleset_UMAP")) +
      geom_boxplot(alpha=.7)+
      geom_jitter(color ="#8B1A1A", size = 0.7, alpha = 0.7) +
      theme_minimal() +
      labs(title = names(covs)[i], x = names(covs)[i])
