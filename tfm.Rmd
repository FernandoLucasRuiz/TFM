...
```{r}
mPC1.pos <- mat_exp[rownames(mat_exp) %in% rownames(outlierspc1_scaled[outlierspc1_scaled[, 1] > 0 , , drop = F]),]
mPC1.neg <- mat_exp[rownames(mat_exp) %in% rownames(outlierspc1_scaled[outlierspc1_scaled[, 1] < 0 , , drop = F ]),]

mPC2.pos <- mat_exp[rownames(mat_exp) %in% rownames(outlierspc2_scaled[outlierspc2_scaled[, 1] > 0 , , drop = F ]),]
mPC2.neg <- mat_exp[rownames(mat_exp) %in% rownames(outlierspc2_scaled[outlierspc2_scaled[, 1] < 0 , , drop = F]),]

# mPC12_scaled <- mat_exp[(rownames(mat_exp) %in% rownames(outlierspc2_scaled)) & (rownames(mat_exp) %in% rownames(outlierspc1_scaled)),]

not_in_mPC12 <- mat_exp[!((rownames(mat_exp) %in% rownames(mPC1.pos)) | (rownames(mat_exp) %in% rownames(mPC1.neg)) | (rownames(mat_exp) %in% rownames(mPC2.pos)) | (rownames(mat_exp) %in% rownames(mPC2.neg))),]
```

```{r}
# Asegurarse de que todos son vectores del mismo largo para el dataframe final
max_length <- max(length(rownames(mPC1.pos)),
                  length(rownames(mPC1.neg)),
                  length(rownames(mPC2.pos)), 
                  length(rownames(mPC2.neg)))

# Normalizar la longitud de los vectores (en caso de que alguno sea más corto)
rownames_mPC1.pos <- c(rownames(mPC1.pos), rep("", max_length - length(rownames(mPC1.pos))))
rownames_mPC1.neg <- c(rownames(mPC1.neg), rep("", max_length - length(rownames(mPC1.neg))))
rownames_mPC2.pos <- c(rownames(mPC2.pos), rep("", max_length - length(rownames(mPC2.pos))))
rownames_mPC2.neg <- c(rownames(mPC2.neg), rep("", max_length - length(rownames(mPC2.neg))))
...
for (i in rownames(covs2)){
  if (i %in% rownames(mPC1.pos) ){
    covs2[i, "sampleset_PCA"] <- "mPC1 positive"
  }
  if (i %in% rownames(mPC1.neg)){
    covs2[i, "sampleset_PCA"] <- "mPC1 negative"
  }
  if (i %in% rownames(mPC2.pos) ){
    covs2[i, "sampleset_PCA"] <- "mPC2 positive"
  }
  if (i %in% rownames(mPC2.neg)){
    covs2[i, "sampleset_PCA"] <- "mPC2 negative"
  }
  if (!(i %in% rownames(mPC1.pos)) & !(i %in% rownames(mPC1.neg)) & !(i %in% rownames(mPC2.pos)) & !(i %in% rownames(mPC2.neg))){
...
for (i in 1:ncol(covs2)) {
  ...
  if (class(covs2[[i]]) == "factor"){
    filt <- covs2 %>%
      ...
      filter(sampleset_PCA == "mPC1 positive" | sampleset_PCA == "mPC1 negative" | sampleset_PCA == "mPC2 positive" | sampleset_PCA == "mPC2 negative") %>%
