...
geneset <- unique(geneset.AD.Andrews$Gene)
```
...
```{r}
mat_exp_genes <- mat_exp[, colnames(mat_exp) %in% geneset]
```

```{r}
venn.plot <- venn.diagram(
  x = list(GenesMatriz = colnames(mat_exp), GenesetAlzheimer = geneset),
...
```{r}
pca_geneset <- prcomp(mat_exp_genes)
```

```{r}
# Scree plot con los datos escalados
var_exp <- pca_geneset$sdev^2
...
```{r}
outlierspc1 <- as.data.frame(pca_geneset$x[abs(pca_geneset$x[,1]) > 1.5*(pca_geneset$sdev[1]),1])

outlierspc2 <- as.data.frame(pca_geneset$x[abs(pca_geneset$x[,2]) > 1.5*(pca_geneset$sdev[2]),2])
```

```{r, fig.height=8, fig.width=12}
df <- as.data.frame(pca_geneset$x)

plot1 <- ggplot(df, aes(x = PC1)) +
  geom_density(fill = "#00CED1", alpha = 0.5) +
  geom_vline(xintercept = mean(df$PC1) + 1.5*pca_geneset$sdev[1], linetype = "dashed", color = "blue") +
  geom_vline(xintercept = mean(df$PC1) - 1.5*pca_geneset$sdev[1], linetype = "dashed", color = "blue") +
  ...
  geom_rug(data = as.data.frame(pca_geneset$x), aes(x= PC1, y = 0), color= ifelse(abs(pca_geneset$x[,1]) > 1.5*(pca_geneset$sdev[1]), "#8B1A1A", "grey")) +
  theme_minimal()

plot2 <- ggplot(df, aes(x = PC2)) +
  geom_density(fill = "#00CED1", alpha = 0.5) +
  geom_vline(xintercept = mean(df$PC2) + 1.5*pca_geneset$sdev[2], linetype = "dashed", color = "blue") +
  geom_vline(xintercept = mean(df$PC2) - 1.5*pca_geneset$sdev[2], linetype = "dashed", color = "blue") +
  labs(title = "PC2 density with 2*SD scaled PCA") +
  geom_text(data = outlierspc2, 
            aes(x = outlierspc2[,1], y= 0, label = rownames(outlierspc2)), 
            vjust = 1.5, hjust = 0, size = 3, color = "#E9965A", angle = 90, fontface= "bold") +
  geom_rug(data = as.data.frame(pca_geneset$x), aes(x= PC2, y = 0), color= ifelse(abs(pca_geneset$x[,2]) > 1.5*(pca_geneset$sdev[2]), "#8B1A1A", "grey")) +
  theme_minimal()
...
```{r}
set.seed(1234)
tsne <- Rtsne(mat_exp_genes, dims = 2, theta = 0.0)

tsne.data <- as.data.frame(tsne$Y)
row.names(tsne.data) <- row.names(mat_exp_genes)
...
```{r}
rownames(tsne.data) <- rownames(mat_exp_genes)
...
set.seed(1234)
umap.ad <- umap(mat_exp_genes,random_stage=1234, local.config)

umap.data <- as.data.frame(umap.ad$layout)
row.names(umap.data) <- row.names(mat_exp_genes)
...
```{r}
rownames(umap.data) <- rownames(mat_exp_genes)
        ...
        for (j in levels(df.covs[[sampleset]])) {
          ...
          if (j == "Not in both") {
            ...
            nuevafila <- data.frame(
              ...
              "mPC_0" = round(expected[1,1], 2),
              "mPC_1" = round(expected[2,1], 2),
              "NOT mPC_0" = round(expected[1,2], 2),
              "NOT mPC_1" = round(expected[2,2], 2),
              stringsAsFactors = FALSE
            )
...
yepa <- calculo.test(df.covs = covs2.casos, sampleset = "sampleset_PCA")
```
...
```{r}
pca_rotation <- round(data.frame(pca_geneset$rotation[,1:2]),3)

reactable(pca_rotation)
```

```{r}
loadings <- pca_geneset$rotation
...
```{r}
pca.var <- get_pca_var(pca_geneset)
...
```{r}
pca.data.kegg <- data.frame(pca_geneset$x)[,1:2]
...
filtered_data <- melted_incidence_matrix %>% 
  filter(presence == 1, total_presence >=2) %>%
...
```{r}
reactable(covs2.casos %>%
  filter(mrna_id %in% names(total_presences[total_presences == 3]))
)
```
