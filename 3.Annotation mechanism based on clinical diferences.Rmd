...
subtitle: "Annotation mechanism based on clinical diferences"
...
# Load data from the outlier detection mechanism

```{r}
load("EXTREME SAMPLE DETECTION MECHANISM.RData")
```

# Annotation mechanism based on clinical diferences

## Covariates in outliers 

Frequency table of HEALTHY AND SICK outliers with PCA
...
Frequency table outliers ONLY AD with PCA
...
Frequency table of HEALTHY AND AD outliers with tSNE
...
Frequency table outliers ONLY AD with tSNE
```{r}
...
```
Table frequency outliers ONLY AD with UMAP
```{r}
      ...
      if (grepl("sampleset_", i) | grepl("batch", i)){
        ...
        for (j in levels(df.covs[[sampleset]])) {
          ...
          if (j == "No outiler") {
            ...
            if (any(expected <= 5)) {
              # Obtain the position of the minimum value
...
```{r}
save.image("Annotation mechanism based on clinical diferences.RData")
```
