...
p <- ggplot(para.plot, aes_string(x = "PC1.KEGG", y = "PC2.KEGG", color = "sampleset_PCA")) +
    ...
    geom_text_repel(aes(label=ifelse(mrna_id %in% rownames(outlierspc1) | mrna_id %in% rownames(outlierspc2),
                                     as.character(gsub("_.*", "", covs2$mrna_id)), "")),
                  color = "black",
...
```{r}
mPC1.Andrews.pos <- rownames(outlierspc1.Andrews[outlierspc1.Andrews[, 1] > 0 , , drop = F])
mPC1.Andrews.neg <- rownames(outlierspc1.Andrews[outlierspc1.Andrews[, 1] < 0 , , drop = F ])

mPC2.Andrews.pos <- rownames(outlierspc2.Andrews[outlierspc2.Andrews[, 1] > 0 , , drop = F ])
mPC2.Andrews.neg <- rownames(outlierspc2.Andrews[outlierspc2.Andrews[, 1] < 0 , , drop = F])
```

```{r}
# Asegurarse de que todos son vectores del mismo largo para el dataframe final
max_length <- max(length(mPC1.Andrews.pos),
                  length(mPC1.Andrews.neg),
                  length(mPC2.Andrews.pos), 
                  length(mPC2.Andrews.neg))

# Normalizar la longitud de los vectores (en caso de que alguno sea más corto)
rownames_mPC1.pos <- c(mPC1.Andrews.pos, rep("", max_length - length(mPC1.Andrews.pos)))
rownames_mPC1.neg <- c(mPC1.Andrews.neg, rep("", max_length - length(mPC1.Andrews.neg)))
rownames_mPC2.pos <- c(mPC2.Andrews.pos, rep("", max_length - length(mPC2.Andrews.pos)))
rownames_mPC2.neg <- c(mPC2.Andrews.neg, rep("", max_length - length(mPC2.Andrews.neg)))
...
tsne.data.Andrews.covs <- merge(tsne.data.Andrews, covs, by = "row.names")
tsne.data.Andrews.covs$Row.names <- NULL
row.names(tsne.data.Andrews.covs) <- tsne.data.Andrews.covs$mrna_id
```
...
```{r}
mtSNE1.Andrews.pos <- rownames(outliers.tsne1.Andrews[outliers.tsne1.Andrews[, 1] > 0, , drop = F])

mtSNE1.Andrews.neg <- rownames(outliers.tsne1.Andrews[outliers.tsne1.Andrews[, 1] < 0, , drop = F])

mtSNE2.Andrews.pos <- rownames(outliers.tsne2.Andrews[outliers.tsne2.Andrews[, 1] > 0, , drop = F])

mtSNE2.Andrews.neg <- rownames(outliers.tsne2.Andrews[outliers.tsne2.Andrews[, 1] < 0, , drop = F])
```

```{r}
# Asegurarse de que todos son vectores del mismo largo para el dataframe final
max_length <- max(length(mtSNE1.Andrews.pos),
                  length(mtSNE1.Andrews.neg),
                  length(mtSNE2.Andrews.pos), 
                  length(mtSNE2.Andrews.neg))

# Normalizar la longitud de los vectores (en caso de que alguno sea más corto)
rownames_mtSNE1.pos <- c(mtSNE1.Andrews.pos, rep("", max_length - length(mtSNE1.Andrews.pos)))
rownames_mtSNE1.neg <- c(mtSNE1.Andrews.neg, rep("", max_length - length(mtSNE1.Andrews.neg)))
rownames_mtSNE2.pos <- c(mtSNE2.Andrews.pos, rep("", max_length - length(mtSNE2.Andrews.pos)))
rownames_mtSNE2.neg <- c(mtSNE2.Andrews.neg, rep("", max_length - length(mtSNE2.Andrews.neg)))
...
# local.config$metric<- "euclidean"
set.seed(1234)
umap.Andrews <- umap(mat.expre.AD.Andrews,random_stage=1234, local.config)

umap.data.Andrews <- as.data.frame(umap.Andrews$layout)
row.names(umap.data.Andrews) <- row.names(mat.expre.AD.Andrews)
umap.data.Andrews.covs <- merge(umap.data.Andrews, covs, by = "row.names")
umap.data.Andrews.covs$Row.names <- NULL
row.names(umap.data.Andrews.covs) <- umap.data.Andrews.covs$mrna_id
```
...
```{r}
mUMAP1.Andrews.pos <- rownames(outliers.umap1.Andrews[outliers.umap1.Andrews[,1] > 0 , , drop = F])

mUMAP1.Andrews.neg <- rownames(outliers.umap1.Andrews[outliers.umap1.Andrews[,1] < 0 , , drop = F])

mUMAP2.Andrews.pos <- rownames(outliers.umap2.Andrews[outliers.umap2.Andrews[,1] > 0 , , drop = F])

mUMAP2.Andrews.neg <- rownames(outliers.umap2.Andrews[outliers.umap2.Andrews[,1] < 0 , , drop = F])
```

```{r}
# Asegurarse de que todos son vectores del mismo largo para el dataframe final
max_length <- max(length(mUMAP1.Andrews.pos),
                  length(mUMAP1.Andrews.neg),
                  length(mUMAP2.Andrews.pos), 
                  length(mUMAP2.Andrews.neg))

# Normalizar la longitud de los vectores (en caso de que alguno sea más corto)
rownames_mtSNE1.pos <- c(mUMAP1.Andrews.pos, rep("", max_length - length(mUMAP1.Andrews.pos)))
rownames_mtSNE1.neg <- c(mUMAP1.Andrews.neg, rep("", max_length - length(mUMAP1.Andrews.neg)))
rownames_mtSNE2.pos <- c(mUMAP2.Andrews.pos, rep("", max_length - length(mUMAP2.Andrews.pos)))
rownames_mtSNE2.neg <- c(mUMAP2.Andrews.neg, rep("", max_length - length(mUMAP2.Andrews.neg)))
...
for (i in rownames(covs2)){
  if (i %in% mPC1.Andrews.pos){
    covs2[i, "sampleset_PCA_Andrews"] <- "mPC1 positive"
  }
  else if (i %in% mPC1.Andrews.neg){
    covs2[i, "sampleset_PCA_Andrews"] <- "mPC1 negative"
  }
  else if (i %in% mPC2.Andrews.pos ){
    covs2[i, "sampleset_PCA_Andrews"] <- "mPC2 positive"
  }
  else if (i %in% mPC2.Andrews.neg){
    covs2[i, "sampleset_PCA_Andrews"] <- "mPC2 negative"
  }
  else {
...
data.frame(table(covs2$sampleset_PCA_Andrews[covs2$neuroStatus == 1]))
```
...
for (i in rownames(covs2)){
  if (i %in% mtSNE1.Andrews.pos){
    covs2[i, "sampleset_tSNE_Andrews"] <- "mtSNE1 positive"
  }
  else if (i %in% mtSNE1.Andrews.neg){
    covs2[i, "sampleset_tSNE_Andrews"] <- "mtSNE1 negative"
  }
  else if (i %in% mtSNE2.Andrews.pos){
    covs2[i, "sampleset_tSNE_Andrews"] <- "mtSNE2 positive"
  }
  else if (i %in% mtSNE2.Andrews.neg){
    covs2[i, "sampleset_tSNE_Andrews"] <- "mtSNE2 negative"
  } else {
...
data.frame(table(covs2$sampleset_tSNE_Andrews[covs2$neuroStatus == 1]))
```
...
for (i in rownames(covs2)){
  if (i %in% mUMAP1.Andrews.pos){
    covs2[i, "sampleset_UMAP_Andrews"] <- "mUMAP1 positive"
  }
  else if (i %in% mUMAP1.Andrews.neg){
    covs2[i, "sampleset_UMAP_Andrews"] <- "mUMAP1 negative"
  }
  else if (i %in% mUMAP2.Andrews.pos){
    covs2[i, "sampleset_UMAP_Andrews"] <- "mUMAP2 positive"
  }
  else if (i %in% mUMAP2.Andrews.neg){
    covs2[i, "sampleset_UMAP_Andrews"] <- "mUMAP2 negative"
  } else {
...
data.frame(table(covs2$sampleset_UMAP_Andrews[covs2$neuroStatus == 1]))
```
...
```{r}
ratios.PCA.Andrews <- data.frame.ratios(covs2.casos, "sampleset_PCA_Andrews")
```

```{r}
ratio.ouliers.vs.no.ouliers <- data.frame(matrix(ncol = 0, nrow = length(rownames(ratios.PCA.Andrews))))

for (i in colnames(ratios.PCA.Andrews)) {
  if (i == "Not in both") {
    next
    
  } else {
    ratio <- data.frame(log2(ratios.PCA.Andrews[[i]] / ratios.PCA.Andrews[["Not in both"]]))
    
    nombre.filas <- rownames(ratios.PCA.Andrews)
...
```{r}
ratios.tsne.Andrews <- data.frame.ratios(covs2.casos, "sampleset_tSNE_Andrews")
```

```{r}
ratio.ouliers.vs.no.ouliers <- data.frame(matrix(ncol = 0, nrow = length(rownames(ratios.tsne.Andrews))))

for (i in colnames(ratios.tsne.Andrews)) {
  if (i == "Not in both") {
    next
    
  } else {
    ratio <- data.frame(log2(ratios.tsne.Andrews[[i]] / ratios.tsne.Andrews[["Not in both"]]))
    
    nombre.filas <- rownames(ratios.tsne.Andrews)
...
```{r}
ratios.umap.Andrews <- data.frame.ratios(covs2.casos, "sampleset_UMAP_Andrews")
```

```{r}
ratio.ouliers.vs.no.ouliers <- data.frame(matrix(ncol = 0, nrow = length(rownames(ratios.umap.Andrews))))

for (i in colnames(ratios.umap.Andrews)) {
  if (i == "Not in both") {
    next
    
  } else {
    ratio <- data.frame(log2(ratios.umap.Andrews[[i]] / ratios.umap.Andrews[["Not in both"]]))
    
    nombre.filas <- rownames(ratios.umap.Andrews)
...
```{r}
pca_rotation_Andrews <- round(data.frame(pca.AD.Andrews$rotation[,1:2]),2)

reactable(pca_rotation_Andrews)
```

```{r}
loadings <- pca.AD.Andrews$rotation

loading.pc1.pos.Andrews <- loadings[loadings[, "PC1"] > 0, "PC1"]
loading.pc1.neg.Andrews <- loadings[loadings[, "PC1"] < 0, "PC1"]
loading.pc2.pos.Andrews <- loadings[loadings[, "PC2"] > 0, "PC2"]
loading.pc2.neg.Andrews <- loadings[loadings[, "PC2"] < 0, "PC2"]

best.loadings.pc1.pos.Andrews <- data.frame(loading = head(sort(loading.pc1.pos.Andrews, decreasing = TRUE), 5))
best.loadings.pc1.neg.Andrews <- data.frame(loading =head(sort(loading.pc1.neg.Andrews, decreasing = FALSE), 5), pc = "mPC1 negative")
best.loadings.pc2.pos.Andrews <- data.frame(loading =head(sort(loading.pc2.pos.Andrews, decreasing = TRUE), 5), pc = "mPC2 positive")
best.loadings.pc2.neg.Andrews <- data.frame(loading =head(sort(loading.pc2.neg.Andrews, decreasing = FALSE), 5), pc = "mPC2 negative")
```


```{r}
pca.var <- get_pca_var(pca.AD.Andrews)
pca.var.coord <- data.frame(pca.var$coord[,1:2])

pca.var.coord.best.loadings <-  pca.var.coord[(rownames(pca.var.coord) %in% rownames(best.loadings.pc1.pos.Andrews)) | (rownames(pca.var.coord) %in% rownames(best.loadings.pc1.neg.Andrews)) | (rownames(pca.var.coord) %in% rownames(best.loadings.pc2.pos.Andrews)) | (rownames(pca.var.coord) %in% rownames(best.loadings.pc2.neg.Andrews)), ]


pca.var.coord.best.loadings <- pca.var.coord.best.loadings %>%
  mutate(PC = case_when(
    row.names(.) %in% rownames(best.loadings.pc1.pos.Andrews) ~ "mPC1 positive",
    row.names(.) %in% rownames(best.loadings.pc1.neg.Andrews) ~ "mPC1 negative",
    row.names(.) %in% rownames(best.loadings.pc2.pos.Andrews) ~ "mPC2 positive",
    row.names(.) %in% rownames(best.loadings.pc2.neg.Andrews) ~ "mPC2 negative",
    TRUE ~ NA_character_  # para cualquier otra condición no especificada
  ))
```
...
covs2_df <- covs2.casos%>%
  dplyr::select(mrna_id, sampleset_PCA_Andrews)
para.plot <- merge(pca.data.Andrews, covs2_df, by = "row.names")
rownames(para.plot) <- para.plot$Row.names
para.plot$Row.names <- NULL

pca.var.coord.best.loadings$id <- rownames(pca.var.coord.best.loadings)

p <- ggplot(para.plot, aes_string(x = "PC1.Andrews", y = "PC2.Andrews", color = "sampleset_PCA_Andrews")) +
    geom_point(show.legend = TRUE, size = 3) +
    geom_text_repel(aes(label=ifelse(mrna_id %in% rownames(outlierspc1) | mrna_id %in% rownames(outlierspc2),
                                     as.character(gsub("_.*", "", covs2$mrna_id)), "")),
                  color = "black",
    ...
    theme(
      ...
      legend.key.size = unit(0.5, "cm"),
      plot.margin = margin(5, 5, 5, 5)) +
  geom_point(data= (covs2 %>% filter(neuroStatus == 0)), aes_string(x = "PC1.Andrews", y = "PC2.Andrews", color = "sampleset_PCA_Andrews"), alpha = 0.5)+ 
  geom_segment(data = pca.var.coord.best.loadings, 
               aes(x= 0, y = 0, xend = Dim.1*12, yend = Dim.2*12, color = PC), 
               arrow = arrow(length = unit(0.05, "inches"), type = "closed"),
               size = 0.4) +
  geom_text_repel(data = pca.var.coord.best.loadings,
                    aes(x = Dim.1*15, 
                        y = Dim.2*15,
                        label=ifelse((id %in% rownames(best.loadings.pc1.pos.Andrews) | id %in% rownames(best.loadings.pc1.neg.Andrews) | id %in% rownames(best.loadings.pc2.pos.Andrews) | id %in% rownames(best.loadings.pc2.neg.Andrews)), id, ""),
                        color = PC),
                  max.overlaps = 10, # Reduce el número máximo de solapamientos
                  size = 3, 
                  fontface = "bold",
