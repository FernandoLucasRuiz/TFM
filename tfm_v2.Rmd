...
plot1 <- ggplot(tsne.data, aes(x = V1)) +
  ...
  geom_vline(xintercept = mean.tsne1 - 1.5*sd.tsne1, linetype = "dashed", color = "blue") +
  labs(title = "tSNE1 density with 1.5*SD tSNE") +
...
plot2 <- ggplot(tsne.data, aes(x = V2)) +
  ...
  geom_vline(xintercept = mean.tsne2 - 1.5*sd.tsne2, linetype = "dashed", color = "blue") +
  labs(title = "tSNE2 density with 1.5*SD tSNE") +
...
### Seleccion genes importantes para mUMAP1+
...
```{r}
important.genes <- varImp(rf.cv.10$`500 trees`)$importance %>% 
  rownames_to_column() %>%
...
important.genes %>%
  ...
  ggplot(aes(x = row_index, y = Overall)) +  # Usar row_index en el eje X
    geom_point() +
    geom_line() +
    labs(x = "Número Genes", title = "Importancia genes en RF") +
    theme_bw() +
    theme(axis.text.x = element_text(size = 10))
...
```{r}
important.genes.top <- important.genes %>%
  head(., 30) 

top_genes <- important.genes.top$rowname

gp_up_ordered = gost(top_genes, organism = "hsapiens",
		        #ordered_query = TRUE, 
		        sources = "GO", 
		        highlight = T)
...
```{r}
top_genes.gprofiler = gconvert(top_genes)


# enrichment analysis using gene names
multi_gp = gost(top_genes.gprofiler$name, 
          multi_query = FALSE, 
          evcodes = TRUE, 
          sources = "GO", 
          highlight = T)
...
```{r}
gp_up_ordered = gost(top_genes, organism = "hsapiens",
		        ordered_query = TRUE, sources = "KEGG")

gostplot(gp_up_ordered, interactive = T)
```
...
enrichplot::dotplot(gp_mod_enrich, showCategory = 20)
```
...
```{r}
gp_up_ordered = gost(top_genes, organism = "hsapiens",
		        ordered_query = TRUE, sources = "REAC")
...
enrichplot::dotplot(gp_mod_enrich, showCategory = 10)
```
