...
subtitle: "Outlier detection mechanism Proteosome"
...
```{r}
load("Annotation mechanism based on biological differences.RData")
```

# Loading proteosome geneset from KEGG

So now I take from KEGG the Proteasome term hsa03050 and perform the same steps to extract the outliers.
...
Filter the matrix Mnxm to Mnxm', with m' << n to filter the predictors, in this case from the expression matrix mat_exp.
...
Now, let's look at the samples that have been detected as extreme cases in both cases. We start first by taking all the outliers whether they are cases or controls. We see that with the ALZHEIMER's mUMAP1+ there are 15 samples that are also in the mUMAP2- of the PROTEASOME geneset.
...
When we filter by the outliers that are CASES, we see that it happens to be the one with the most combinations between them with 12 patients in common.
